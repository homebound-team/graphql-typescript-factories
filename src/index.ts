...
import { convertFactory, ConvertFn, RawConfig } from "@graphql-codegen/visitor-plugin-common";

const DEFAULT_IMPORT_NAME = "GraphQLAutoGeneratedTypes";

class CodeResolver {
  private config: Config;
  private convertFunction: ConvertFn;

  constructor(config: Config) {
    this.config = config;
    this.convertFunction = convertFactory(config);
  }

  maybeImport(typeName: string): Code {
    return code`${!!this.config.typesFilePath ? imp(`${typeName}@${this.config.typesFilePath}`) : typeName}`;
  }

  getRealImportedEnum(detailObject: GraphQLOutputType): Code {
    return this.maybeImport(getRealEnumForEnumDetailObject(detailObject).name);
  }

  getQualifiedEnumValue(type: GraphQLEnumType, value: GraphQLEnumValue): Code {
    return code`${this.maybeImport(type.name)}.${this.convertFunction(value.astNode || value.value)}`;
  }

  getDefaultValue(type: GraphQLScalarType): Code | string {
    const defaultFromConfig = this.config.scalarDefaults[type.name];
...
export const plugin: PluginFunction = async (schema, documents, config: Config) => {
  ...
  const codeResolver = new CodeResolver(config);

  generateFactoryFunctions(codeResolver, schema, interfaceImpls, chunks);
  generateInterfaceFactoryFunctions(codeResolver, interfaceImpls, chunks);
  generateEnumDetailHelperFunctions(codeResolver, schema, chunks);
...
function generateFactoryFunctions(
  ...
  Object.values(schema.getTypeMap()).forEach((type) => {
    if (shouldCreateFactory(type)) {
      chunks.push(...newFactory(codeResolver, interfaceImpls, type));
    }
  });
}

function generateInterfaceFactoryFunctions(codeResolver: CodeResolver, interfaceImpls: Record<string, string[]>, chunks: Code[]) {
  Object.entries(interfaceImpls).forEach(([interfaceName, impls]) => {
    chunks.push(...newInterfaceFactory(codeResolver, interfaceName, impls));
  });
}

/** Makes helper methods to convert the "maybe enum / maybe enum detail" factory options into enum details. */
function generateEnumDetailHelperFunctions(codeResolver: CodeResolver, schema: GraphQLSchema, chunks: Code[]) {
  ...
  usedEnumDetailTypes.forEach((type) => {
    ...
    const scopedEnumTypeName = codeResolver.maybeImport(enumType.name);
    const scopedTypeName = codeResolver.maybeImport(type.name);
...
function newFactory(
  ...
  const scopedName = codeResolver.maybeImport(type.name);
  ...
  const optionFields: Code[] = Object.values(type.getFields()).map((f) => {
    ...
    if (fieldType instanceof GraphQLObjectType && isEnumDetailObject(fieldType)) {
      return code`${f.name}?: ${fieldType.name}Options | ${codeResolver.getRealImportedEnum(fieldType)}${orNull};`;
  ...
  const typeImp = codeResolver.maybeImport(type.name);
      ...
      ${Object.values(type.getFields()).map((f) => {
        if (f.type instanceof GraphQLNonNull) {
          ...
          if (isEnumDetailObject(fieldType)) {
            ...
            return code`o.${f.name} = options.${f.name} ?? ${getInitializer(codeResolver, type, f, fieldType)};`;
          }
...
/** Creates a `new${type}` function for the given `type`. */
function newInterfaceFactory(codeResolver: CodeResolver, interfaceName: string, impls: string[]): Code[] {
  ...
  return [
    ...
    code`
      ...
      impls.map((type) => codeResolver.maybeImport(type)),
      { on: " | " },
    )};
    `,
    ...
    code`
      ...
      function maybeNewOrNull${interfaceName}(value: ${interfaceName}Options | undefined | null, cache: Record<string, any>): ${codeResolver.maybeImport(interfaceName)} | null {
        if (!value) {
          return null;
        } else if (value.__typename) {
          return value as ${interfaceName}Type;
...
function getInitializer(
  ...
  if (type instanceof GraphQLList) {
    ...
    return codeResolver.getQualifiedEnumValue(type, type.getValues()[0]);
    ...
    return codeResolver.getDefaultValue(type);
  }
